...
object Main  {
  ...
  private val prover_output =
    Session.Consumer[Session.Commands_Changed](getClass.getName) {
      case changed => println(s"prover> ${changed.nodes.toList.map( f=> f.node)}")
    }
      ...
      Isabelle_Thread.fork(name = "blah", inherit_locals = true) {
        val rc =
          try {
            ...
            session.prover_me            val res1 = session.use_theories(List("First_Order_Logic"), master_dir="/home/eingerman/Projects/DeepMath/Isabelle2021/src/Pure/Examples/", progress = progress)
            // println(res1.state)
            // println(res1.nodes)
